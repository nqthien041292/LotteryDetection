...
public class GraphQLTestBase<TSchema, TDocumentBuilder> : AppTestBase<LotteryDetectionGraphQLTestModule>
    where TSchema : ISchema
    where TDocumentBuilder : IDocumentBuilder, new()
{
    ...
    public ExecutionResult ParseQueryResult(string result, ExecutionErrors errors = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(result)) data = JsonNode.Parse(result);
