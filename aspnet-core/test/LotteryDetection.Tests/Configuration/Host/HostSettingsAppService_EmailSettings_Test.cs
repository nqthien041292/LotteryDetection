...
using LotteryDetection.Configuration.Host;
using Shouldly;

namespace LotteryDetection.Tests.Configuration.Host;
...
public class HostSettingsAppService_EmailSettings_Test : AppTestBase
{
    ...
    public async Task Should_Change_Email_Settings()
    {
        ...
        (await _settingManager.GetSettingValueAsync(EmailSettingNames.DefaultFromDisplayName)).ShouldBe(
            "My daily mailing service");
        ...
        (await _settingManager.GetSettingValueAsync<bool>(EmailSettingNames.Smtp.UseDefaultCredentials))
            .ShouldBe(true); //not changed
    }
}