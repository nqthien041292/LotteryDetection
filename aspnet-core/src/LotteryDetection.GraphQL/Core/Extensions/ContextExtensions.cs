...
namespace LotteryDetection.Core.Extensions;
...
public static class ContextExtensions
{
        ...
        Action<TArgType> argumentContainsAction)
    {
        if (context.Arguments.ContainsKey(argumentName))
        {
            ...
            if (argument.Source == ArgumentSource.FieldDefault) return context;
        ...
        char namespaceSeperator = ':')
    {
        ...
        if (context.SubFields[fragments[0]].Field == null) return false;

        if (context.SubFields[fragments[0]].Field?.SelectionSet == null) return false;

        if (context.SubFields[fragments[0]].Field?.SelectionSet.Selections == null) return false;
