...
public class WidgetDefinitionConverter : JsonConverter<WidgetDefinition>
{
    public override WidgetDefinition Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        string id = null;
        if (jsonElement.TryGetProperty(nameof(WidgetDefinition.Id), out var idProperty)) id = idProperty.GetString();
        ...
        if (jsonElement.TryGetProperty(nameof(WidgetDefinition.AllowMultipleInstanceInSamePage),
                out var allowMultipleInstanceInSamePageProperty))
        ...
        if (jsonElement.TryGetProperty(nameof(WidgetDefinition.PermissionDependency),
                out var permissionDependencyProperty))
        {
    ...
    public override void Write(Utf8JsonWriter writer, WidgetDefinition value, JsonSerializerOptions options)
    {
        ...
        if (value.PermissionDependency != null)
        {
            ...
            // writer.WriteStringValue(JsonSerializationHelper.SerializeWithType(value.PermissionDependency, typeof(LotteryDetectionSimplePermissionDependency)));
        }
