...
public class QrLoginManager : LotteryDetectionDomainServiceBase, IQrLoginManager
{
    ...
    public async Task<bool> VerifySessionId(string connectionId, string sessionId)
    {
        ...
        if (cache == null) throw new UserFriendlyException(L("QrLoginConnectionIdNotFoundCache"));

        if (cache.Code != sessionId) throw new UserFriendlyException(L("WrongQrLoginSessionId"));
