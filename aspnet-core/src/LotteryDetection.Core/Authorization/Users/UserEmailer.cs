...
/// </summary>
public class UserEmailer : LotteryDetectionServiceBase, IUserEmailer, ITransientDependency
{
    ...
    public async Task SendPasswordResetLinkAsync(User user, string link = null)
    {
        ...
        if (!link.IsNullOrEmpty())
        {
            ...
            var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours)
                .ToString(LotteryDetectionConsts.DateTimeOffsetFormat, CultureInfo.InvariantCulture));
    ...
    {
        if (code.IsNullOrEmpty()) throw new Exception("Code should be set in order to send passwordless code link.");
        ...
        mailMessage.AppendLine("<br />");
        mailMessage.AppendLine("<br />");
        mailMessage.AppendLine(string.Format(L("PasswordlessCode_ExpirationDescription"),
            PasswordlessLoginCodeCacheItem.DefaultSlidingExpireTime.TotalMinutes));
        mailMessage.AppendLine("<br />");
