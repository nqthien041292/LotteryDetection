...
public class EmailSettingsChecker : IEmailSettingsChecker, ITransientDependency
{
    ...
    {
        if (DebugHelper.IsDebug) return true;
        ...
        if (defaultFromAddress.IsNullOrEmpty() || host.IsNullOrEmpty()) return false;

        // these are dummy values ASP.NET Zero creates initially
        if (defaultFromAddress == "admin@mydomain.com" || host == "mydomain.com mailer") return false;

        if (_settingManager.GetSettingValue<bool>(EmailSettingNames.Smtp.UseDefaultCredentials)) return true;
    ...
    public async Task<bool> EmailSettingsValidAsync()
    {
        ...
        if (await _settingManager.GetSettingValueAsync<bool>(EmailSettingNames.Smtp.UseDefaultCredentials)) return true;
