...
using LotteryDetection.DataExporting.Excel.MiniExcel;
    ...
    public async Task<FileDto> ExportToFile(List<AuditLogListDto> auditLogList)
    {
        ...
        foreach (var auditLog in auditLogList)
            items.Add(new Dictionary<string, object>
            {
                {
                    L("Time"),
                    _timeZoneConverter.Convert(auditLog.ExecutionTime, _abpSession.TenantId, _abpSession.GetUserId())
                },
    ...
    public async Task<FileDto> ExportToFile(List<EntityChangeListDto> entityChangeList)
    {
        ...
        foreach (var entityChange in entityChangeList)
            items.Add(new Dictionary<string, object>
            {
                ...
                {
                    L("Time"),
                    _timeZoneConverter.Convert(entityChange.ChangeTime, _abpSession.TenantId, _abpSession.GetUserId())
                }
            });
