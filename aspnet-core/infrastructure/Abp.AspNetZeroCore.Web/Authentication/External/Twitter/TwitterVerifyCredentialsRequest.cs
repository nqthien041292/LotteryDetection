...
public class TwitterVerifyCredentialsRequest : TwitterRequestBase
{
    ...
    public override string GetParameterString(string parameterSeparator)
    {
        ...
        return "oauth_consumer_key=" + WebUtility.UrlEncode(ConsumerKey) + parameterSeparator + "oauth_nonce=" +
               ...
               WebUtility.UrlEncode(totalSeconds.ToString()) + parameterSeparator + "oauth_token=\"" +
               WebUtility.UrlEncode(Token) + "\"" + parameterSeparator + "oauth_version=" + WebUtility.UrlEncode("1.0");
    }
}